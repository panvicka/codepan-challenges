...
function calculateDistanceFromCoordinates(xA, yA, xB, yB) {
...
var MINIMAL_DIV_WIDTH = 70;
var MAXIMAL_DIV_WIDTH = 90;
var MINIMAL_DIV_HEIGHT = 40;
var MAXIMAL_DIV_HEIGH = 80;
var NUMBER_OF_DIVS = 60;
var DISTANCE_MINIMAL = 80;
var DISTANCE_MAXIMAL = 250;
function generateDivs() {
    for (var i = 0; i < NUMBER_OF_DIVS; i++) {
        var randomWidth = Math.floor(Math.random() * (MAXIMAL_DIV_WIDTH - MINIMAL_DIV_WIDTH) +
            MINIMAL_DIV_WIDTH);
        var randomheight = Math.floor(Math.random() * (MAXIMAL_DIV_HEIGH - MINIMAL_DIV_HEIGHT) +
            MINIMAL_DIV_HEIGHT);
        var randomTop = Math.floor(Math.random() * (containerDiv.clientHeight - randomheight));
        var randomLeft = Math.floor(Math.random() * (containerDiv.clientWidth - randomWidth));
...
function handleDivMovement(div) {
    var randomTopOffset = Math.floor(Math.random() * 5);
    var randomLeftOffset = Math.floor(Math.random() * 10);
    var newTopPosition = div.offsetTop - mouseInfo.yDiff + randomTopOffset;
    var newLeftPosition = div.offsetLeft - mouseInfo.xDiff + randomLeftOffset;
...
function calculateDistanceFromPointer(div) {
    return calculateDistanceFromCoordinates(mouseInfo.lastX, mouseInfo.lastY, div.getBoundingClientRect().x + div.clientHeight / 2, div.getBoundingClientRect().y + div.clientWidth / 2);
}
function handleMouseMove(e) {
    saveMousePosition(e);
    var allDivs = document.querySelectorAll(".test-div");
    allDivs.forEach(function (div) {
        var distanceFromPointer = calculateDistanceFromPointer(div);
        if (distanceFromPointer <
